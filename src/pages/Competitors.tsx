...
export function Competitors({ analyzedItems, competitorPrices, refreshRecommendations }: CompetitorsProps) {
  ...
  const comparisonData = useMemo(() => {
    ...
    return items.map((item) => {
      ...
      const avgCompPrice = prices.length > 0 ? prices.reduce((s, p) => s + p.price, 0) / prices.length : item.salePrice
    ...
    return (
      <div className="text-center py-20 animate-fade-in">
        ...
        <p className="text-sm text-[var(--color-text-secondary)]">
          Add menu items first, then see how your prices stack up.
        </p>
      </div>
    )
  }
    ...
    <div className="space-y-6 animate-fade-in">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span
                  className={cn(
                    ...
                    platformColors[prices[0]?.platform] || 'bg-gray-50 text-gray-600',
                  )}
                >
                  {prices[0]?.platform}
                </span>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[var(--color-surface-tertiary)] border-b border-[var(--color-border)]">
                <th className="text-left px-4 py-3 font-semibold text-xs uppercase tracking-wider text-[var(--color-text-secondary)]">
                  Item
                </th>
                <th className="text-right px-4 py-3 font-semibold text-xs uppercase tracking-wider text-[var(--color-text-secondary)]">
                  Your Price
                </th>
                {competitors.map((name) => (
                  <th
                    key={name}
                    className="text-right px-4 py-3 font-semibold text-xs uppercase tracking-wider text-[var(--color-text-secondary)]"
                  >
                    {name}
                  </th>
                ))}
                ...
                <th className="text-center px-4 py-3 font-semibold text-xs uppercase tracking-wider text-[var(--color-text-secondary)]">
                  Position
                </th>
              </tr>
            </thead>
      ...
      <div className="flex flex-wrap gap-4 text-xs text-[var(--color-text-secondary)]">
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-emerald-500" /> Below competitors (room to raise)
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-red-500" /> Above competitors (review pricing)
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-gray-400" /> In line with market
        </span>
      </div>
    </div>
  )
