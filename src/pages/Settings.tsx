        ...
        {tabs.map(tab => (
          <button
            ...
            className={cn(
              ...
              activeTab === tab.id ? 'bg-white shadow-sm text-[var(--color-text-primary)]' : 'text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)]'
            )}
          >
            <tab.icon size={15} />
            {tab.label}
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-[var(--color-border)] p-6">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="settings-restaurant-name" className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">Restaurant Name</label>
                <input
              ...
              <div className="opacity-50">
                <label htmlFor="settings-currency" className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">Currency <span className="text-[var(--color-text-tertiary)]">(Coming soon)</span></label>
                <select
          ...
          <div className="bg-white rounded-2xl border border-[var(--color-border)] p-6">
            ...
            <p className="text-xs text-[var(--color-text-tertiary)] mb-3">These defaults will apply to new menu items once backend persistence is connected.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 opacity-50">
              <div>
                <label htmlFor="settings-labor-rate" className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">Default Labor Rate ($/min) <span className="text-[var(--color-text-tertiary)]">(Coming soon)</span></label>
                <input
              ...
              <div>
                <label htmlFor="settings-waste" className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">Default Waste Factor (%) <span className="text-[var(--color-text-tertiary)]">(Coming soon)</span></label>
                <input
                  id="settings-waste"
                  type="number" step="1"
                  value={defaultWaste}
      ...
      {activeTab === 'integrations' && (
        <div className="space-y-4">
          {[
            ...
            <div key={integration.name} className="bg-white rounded-2xl border border-[var(--color-border)] p-5 flex items-center justify-between">
      ...
      {activeTab === 'notifications' && (
        <div className="bg-white rounded-2xl border border-[var(--color-border)] p-6 space-y-5">
          ...
          {[
            ...
            <div key={i} className="flex items-center justify-between py-3 border-b border-[var(--color-border-light)] last:border-0 opacity-50">
              <div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-600/80 mt-1 leading-relaxed">
              This app uses Supabase for authentication and data storage. Add your <code className="bg-indigo-100 px-1 rounded">VITE_SUPABASE_URL</code> and <code className="bg-indigo-100 px-1 rounded">VITE_SUPABASE_ANON_KEY</code> environment variables to connect to your backend. Currently running in demo mode with sample data.
            </p>
          </div>
        </div>
      </div>
